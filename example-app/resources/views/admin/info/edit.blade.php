        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div style="padding: 20px;">
                    <form action="{{route('update_info',$in->id)}}" method="post">
                        @csrf
                        @method('PUT')
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                <label for="inputEmail4">Title</label>
                                <input type="text" class="form-control" name="title" id="title" value="{{$in->title}}">
                            </div>
                            <div class="form-group col-md-6">
                                <label for="inputPassword4">Type</label>
                                <input type="number" class="form-control" id="type" name="type" value="{{$in->type}}">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="inputAddress">Content</label>
                            <textarea class="form-control" id="ckeditor" name="content" rows="5">{!!$in->content!!} </textarea>
                        </div>

                </div>
                <button type="submit" class="btn btn-primary" style="margin-left: 20px;">Thực hiện</button> <a href="{{route('info')}}" style="padding: 2%;"><i style="font-size:24px" class="fa">&#xf122;</i></a>
                </form>
            </div>
