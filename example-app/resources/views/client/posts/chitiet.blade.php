        ...
        <div class="container text-center py-5">
            ...
            <nav aria-label="breadcrumb animated slideInDown">
                <ol class="breadcrumb justify-content-center mb-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{route('all_post')}}">{{ trans('messages.tintuc') }}</a></li>
                    @if($ct_post)
                    <li class="breadcrumb-item text-dark" aria-current="page">{{$ct_post->title}}</li>
                    @endif
                </ol>
            </nav>
        </div>
        ...
        <div class="container">
            <div class="row g-5">
                <div class="col-lg-5 wow fadeIn" data-wow-delay="0.1s">
                    <img class="img-fluid" src="{{asset('upload/post/'.$ct_post->thumbnail)}}" alt="">
                </div>
                <div class="col-lg-6 wow fadeIn" data-wow-delay="0.5s">
                    <div class="section-title">
                        <h1 class="display-6">{{$ct_post->title}}</h1>
                    </div>
                    <span class="mb-4" style="word-wrap: break-word;overflow: hidden;">{!! $ct_post->content !!}</span>
                    <a href="{{route('all_post')}}" class="btn btn-primary rounded-pill py-3 px-5">Back</a>
                </div>
            </div>
        </div>
    </div>
    @else
    @if(Session::get('local')=='vi')
    <p style="text-align: center; margin-top: 30px; color: red; font-size: 20px;">Không có bài viết viết</p>
    @else
    <p style="text-align: center; margin-top: 30px; color: red; font-size: 20px;">There are no posts </p>
    @endif
    @endif
    <!-- Contact Start -->
